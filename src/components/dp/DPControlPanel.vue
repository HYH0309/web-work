...
<template>
  <BaseControlPanel direction="horizontal">
    ...
    <Menu as="div" class="relative">
      <MenuButton class="btn-lg btn flex items-center gap-2 bg-background text-text" :disabled="store.state.isDPing">
    ...
    <!-- 操作按钮组 -->
    <template #actions>
      <button class="btn-md btn bg-blue border-none" @click="store.startDPing()" :disabled="store.state.isDPing">
        开始求解
      </button>
      <button class="btn-md btn bg-yellow border-none" @click="store.nextStep()"
        :disabled="!store.state.isDPing || !store.state.isManualMode">
        下一步
      </button>
      <button class="btn-md btn bg-green border-none" @click="store.state.isManualMode = !store.state.isManualMode"
        :class="{ manual: store.state.isManualMode }" :disabled="store.state.isDPing">
        {{ store.state.isManualMode ? '手动模式' : '自动模式' }}
      </button>
      <button class="btn-md btn bg-red border-none" @click="store.resetMatrix()" :disabled="store.state.isDPing">
        重置
      </button>
